...
function(input, output, session) {
  ...
  addBond <- reactive({
    ...
    freqInput <- ifelse(frequency == "Semi-Annual",
                        2,
                        ifelse(frequency == "Zero-Coupon", 0, 1))
    couponRate <- if (freqInput == 0)
      0
    else
      input$coupon_rate
    ...
    bondInfoVec <- c(faceValue,
                     as.numeric(couponRate / 100),
                     maturity,
                     freqInput)
    bondInfovec <- as.numeric(bondInfoVec)

    keySensToday <- list(
      computeKeySens(spots, bondInfoVec) %>%
        dplyr::rename(
          !!paste0("Bond ", bond_id, " Delta") := "delta",!!paste0("Bond ", bond_id, " Gamma") := "gamma"
        )
    )

    spotsYesterday <- getSplineSpot(maturity, freqInput, spotsKeyYesterday)

    bondPriceYesterday <- priceBond(faceValue,
                                    (couponRate / 100),
                                    maturity,
                                    freqInput,
                                    spotsYesterday)
  ...
  output$portfolio_pie <- renderPlotly({
    ...
    portData <- portfolio() %>%
      dplyr::mutate(label = paste0("Bond ", bond_id),
                    weight = pv / sum(pv))
    ...
    plotly::plot_ly(
      ...
      plotly::layout(title = "Portfolio Weights by Bond Value", showlegend = TRUE)
  }) %>%
  ...
  output$exposure_table <- render_gt({
    ...
    sensitivityTable %>%
      ...
      tab_header(title = "Bonds + Portfolio Delta Exposures")
  })
  ...
  plAttribCalc <- reactive({
    ...
    predMaturities <- data.frame(maturity = sensDfYest$maturity)
    spotRatesToday <- spotSplineToday %>%
      stats::predict(newdata = predMaturities)

    spotRatesYesterday <- spotSplineYesterday %>%
      stats::predict(newdata = predMaturities)

    spotRatesYest <- tibble(maturity = sensDfYest$maturity, rate = spotRatesYesterday)

    spotRatesTod <- tibble(maturity = sensDfYest$maturity, rate = spotRatesToday)
  ...
  output$pnl_by_rate <- render_gt({
    ...
    attribPnL %>%
      ...
      dplyr::rename("Time to Maturity" = maturity) %>%
      gt() %>%
      tab_header(title = "Sensitivites at Select Maturities", subtitle = "** Only select rates displayed. Not a complete picture of PL Attribution **") %>%
  ...
  output$yield_curve_dynamics <- renderPlotly({
    ...
    p %>%
      plotly::layout(
        title = list(text = "U.S. Treasury Yield Curve Dynamics", font = list(size = 18)),
        xaxis = list(title = "Date", type  = "date"),
  ...
  corr_data <- reactive({
    ...
    ir.wide %>%
      dplyr::filter(date >= input$corr_date_selection[1],
  ...
  output$corr_table <- render_gt({
    ...
    as.data.frame(matrix1) %>%
      ...
      tab_header(
        title = paste0(
          "Correlation Matrix - ",
          input$corr_date_selection[1],
          " to ",
          input$corr_date_selection[2]
        )
      ) %>%
      fmt_number(columns = -Maturity, decimals = 2) %>%
      data_color(
        columns  = -Maturity,
